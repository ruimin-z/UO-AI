...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        if not legalActions:
            return None
        if util.flipCoin(self.epsilon): # if r < e, then random action
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Q(s,a) = (1-alpha) * Q(s,a) + alpha * [R + discount * max_{a'} Q(s',a')]
        Q = self.getQValue(state,action)
        sample = reward + self.discount * self.getValue(nextState)
        self.values[(state,action)] = (1- self.alpha) * Q + self.alpha * sample
